/**
 * @file app/middlewares/auth/auth.middlewares.js
 * @description 인증 및 인가 처리 미들웨어
 * 251126 v1.0.0 Kim init 
 */
...
function authorize(req) {
  ...
  const matchRole = ROLE_PERMISSIONS[req.method].find(item => {
    ...
    //   req.baseUrl, // 프리픽스 묶은 path, `/api/auth`
    //   req.path     // `baseUrl`을 제외한 `/login`
    // );
